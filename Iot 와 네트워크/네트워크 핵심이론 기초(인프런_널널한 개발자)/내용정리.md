...
1. MTU 의 크기 차이로 인해 발생한다. 
   1. 즉, Packet의 크기가 1500Byte인데, 특정 라우터의 MTU 가 1400Byte 라면 단편화가 발생한다. 
   2. MTU 가 1400Byte인 경우 = VPN
...
2. Dynamic이란 네트워크에서는 Runtime과 같다. 
...
3. 동작과정 
   ...
   4. **Brodcast 전송**
      1. DHCP 설정시 Broadcast 를 날려 DHCP Server를 찾게된다. 
...
3. Host 주소 설정 
   1. Frame Header에 **목적지 Mac 주소**는 Naver의 Mac 주소가 아닌 **Gateway의 Mac 주소**이다. 
...
1. Ping
   1. 유틸리티( 측정 도구 )
   2. 특정 Host에 대한 RTT 를 측정할 목적으로 사용됨 
...
1. TCP
   1. 연결지향 (순서번호)
   2. TCP Connection , TCP Session, Vertual Circule 으로 불린다. 
   3. 연결 + 상태(전이) 개념이 붙어다님 
...
1. `TIME_WAIT` 는 "연결 최종 완료 전" 에 발생한다. 
   1. 마지막 ACK을 보내면 연결 끊김
2. `CLOSED` 상태가 되면 Socket(자원) 이 회수된다. 
