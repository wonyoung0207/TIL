...
| **`clamp(x, a, b)`**               | `x`를 `[a(min),b(max)]` 범위로 제한  | `clamp(1.5, 0.0, 1.0)` → `1.0`                    |
| **`mix(a, b, t)`**                 | 보간 (t 값에 따라 a ~ b 사이로 보간) | `mix(0.0, 1.0, 0.5)` → `0.5`                      |
| **`step(edge, x)`**                | 기준값edge 기준 0/1 반환(작으면0)    | `step(0.5, 0.3)` → `0.0`                          |
